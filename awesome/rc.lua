...
function run_once(prg)
      awful.util.spawn_with_shell("ps aux | grep '" .. prg .. "' | grep -v 'grep' || (" .. prg .. ")")
...
beautiful.init("/home/chuda/.config/awesome/themes/zenburn/theme.lua")
...
tags = {}
tags_layouts = { layouts[10], layouts[10], layouts[2],
                 layouts[1], layouts[1], layouts[10],
                 layouts[7], layouts[7], layouts[10]
                }
for s = 1, screen.count() do
    -- Each screen has its own tag table.
    tags[s] = awful.tag({ 1, 2, 3, 4, 5, 6, 7, 8, 9 }, s, tags_layouts)
end
-- }}}
...
local mpc_state = {cid = nil}

local function mpc_show_msg(label)
    mpc_state.cid = naughty.notify({ text = label,
                        icon = "/usr/share/icons/Tango/32x32/mimetypes/audio-x-generic.png",
                        timeout = 3,
                        screen = mouse.screen, -- important, not all screens may be visible
                        font = "free sans 18",
                        replaces_id = mpc_state.cid }).id
end

local function mpc_current()
    local label = ""

    local xrandr = io.popen("mpc -f '%artist% - %title%'")
    if xrandr then
...
local function mpc_random()
    ...
    local xrandr = io.popen("mpc random | grep -o 'random: [^ \t]*' | cut -d ' ' -f 2")
    if xrandr then
        for line in xrandr:lines() do
            label = label .. line
        end
    ...
    mpc_show_msg("[random: " .. label .. "]")
end

local function mpc_toggle()
    local label = ""

    local xrandr = io.popen("mpc toggle | awk 'NR == 2 {print $1}'")
    if xrandr then
        for line in xrandr:lines() do
            label = label .. line
        end
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,         }, "'",              function () awful.menu.clients({ width= 250 }, { keygrabber=true }) end)
)
